...
import geckordp.tests.helpers.utils as utils
...
def test_linear_buffer_append_get():
    ...
    string = buffer.get().tobytes().decode(encoding="utf-8").split("\0", 1)[0]
    assert string == "hÈllo"
...
def test_linear_buffer_append_get_null_terminated():
    buffer = LinearBuffer(11)
    append(buffer)
    string = buffer.get_null_terminated().tobytes().decode(encoding="utf-8")
    assert string == "hÈllo"
...
def test_linear_buffer_append_get_truncated():
    buffer = LinearBuffer(11)
    append(buffer)
    string = buffer.get_truncated(3).tobytes().decode(encoding="utf-8")
    assert string == "hÈ"
...
def test_linear_buffer_reset():
    ...
    # note: decode doesn't handle null termination
    string = data.tobytes().decode(encoding="utf-8").split("\0", 1)[0]
